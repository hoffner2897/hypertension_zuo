...
export const actionAdviceSchema = z.object({
  diet: z.object({
    structure: z.string().trim().min(1).max(180),
    cooking: z.string().trim().min(1).max(180)
  }).strict(),
  exercise: z.object({
    timing: z.string().trim().min(1).max(180),
...
export function buildActionAdviceEvidence(timeZone: string, meals: ActionAdviceMeal[], exercises: ActionAdviceExercise[]): ActionAdviceEvidence {
  ...
  return {
    ...
    exerciseSummary: {
      ...
      byTimeBucket: groupStats(uniqueExercises, (item) => timeBucket(item.scheduledStartAt, timeZone)).map(([bucket, values]) => ({
...
function timeBucket(value: string, timeZone: string): string {
...
function oneOrTwoSentences(value: string): string {
  ...
  const sentences = text.split(/(?<=[。！？])/).filter(Boolean).slice(0, 2).join("");
  const result = sentences || text;
  return result.length <= 180 ? result : `${result.slice(0, 179)}…`;
}
