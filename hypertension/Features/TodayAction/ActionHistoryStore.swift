...
enum ActionHistoryStore {
    static func restoreToday(_ items: [TodayActionItem], userId: String, now: Date = Date()) -> [TodayActionItem] {
        ...
        var restoredItems = items.map { item in
            ...
            restored.status = TodayActionStatus(actionHistoryAPIValue: observation.status) ?? item.status
        ...
        restoredItems.append(contentsOf: saved.compactMap { observation in
            TodayActionItem(actionHistoryObservation: observation)
        })
...
private extension TodayActionItem {
    init?(actionHistoryObservation observation: StoredActionObservation) {
        ...
        self.init(
            ...
            status: TodayActionStatus(actionHistoryAPIValue: observation.normalizedStatus) ?? .pending,
